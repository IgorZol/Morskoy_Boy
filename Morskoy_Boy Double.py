    ...
    def draw_grid(self):
        for i in range(7):
            ...
            pygame.draw.line(screen, BLACK, (left_margin+self.offset, upper_margin+i*block_size),
                             (left_margin+6*block_size+self.offset, upper_margin+i*block_size), 1)
            # Vertical lines
            pygame.draw.line(screen, BLACK, (left_margin+i*block_size+self.offset, upper_margin),
                             (left_margin+i*block_size+self.offset, upper_margin+6*block_size), 1)
    ...
    def add_nums_letters_to_grid(self):
        for i in range(6):
            ...
            screen.blit(letters_hor, (left_margin + i*block_size + (block_size //
                                                                    2 - letters_hor_width//2)+self.offset, upper_margin + 6*block_size))
...
class ShipsOnGrid:
    def __init__(self):
        self.available_blocks = {(a, b) for a in range(1, 7)
                                 for b in range(1, 7)}
        self.ships_set = set()
    ...
    def add_block_to_ship(self, coor, str_rev, x_or_y, ship_coordinates):
        if (coor <= 1 and str_rev == -1) or (coor >= 6 and str_rev == 1):
            str_rev *= -1
    ...
    def update_available_blocks_for_creating_ships(self, new_ship):
        for elem in new_ship:
            for k in range(-1, 2):
                for m in range(-1, 2):
                    if 0 < (elem[0]+k) < 7 and 0 < (elem[1]+m) < 7:
                        self.available_blocks.discard((elem[0]+k, elem[1]+m))

    def populate_grid(self):
        ships_coordinates_list = []
        for number_of_blocks in range(3, 0, -1):
            for _ in range(4-number_of_blocks):
...
def draw_ships(ships_coordinates_list):
    for elem in ships_coordinates_list:
        ...
        if ships_coordinates_list == human.ships:
            x += 9 * block_size
        pygame.draw.rect(
            screen, BLACK, ((x, y), (ship_width, ship_height)), width=block_size//6)
...
def put_dot_on_missed_block(fired_block, computer_turn=False):
    if not computer_turn:
        ...
        dotted_set.add((fired_block[0] + 9, fired_block[1]))
...
def main():
    ...
    human_grid = Grid("HUMAN", 9*block_size)
    ...
    while not game_over:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                if (left_margin <= x <= left_margin + 6*block_size) and (upper_margin <= y <= upper_margin + 6*block_size):
